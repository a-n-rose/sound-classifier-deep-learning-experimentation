...
#feed data to models
class Generator:
    def __init__(self,model_type,data,timesteps,step_size,frame_width):
        '''
        ...
        while 1:
            ...
            if 'lstm' == self.model_type.lower():
                ...
                #Only for the CNN+LSTM
                    ...
                    #sliding window, overlapping by step_size 
                    if self.step_size == 0: #no overlap
                        X_batch = X_batch.reshape((self.timesteps,self.frame_width,X_batch.shape[1],1))
                    else:
                        X_batch = np.array([X_batch[i:i+self.frame_width] for i in range(0,len(X_batch)-self.frame_width,self.step_size)])
